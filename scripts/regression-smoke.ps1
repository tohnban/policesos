...
$phpFiles = Get-ChildItem -Path "$repoRoot/app", "$repoRoot/src", "$repoRoot/public", "$repoRoot/config" -Recurse -File -Filter *.php |
    Where-Object { $_.FullName -notlike '*\\src\\vendor\\*' }
...
Assert-Contains -Path "$repoRoot/src/classes/ClassRoutes.php" -Pattern '"moderate_users"=>"ControllerDashboard"' -Description 'rota snake_case de moderacao'
...
# No inline <script> blocks (without src=) allowed in views.
$inlineScriptFiles = Get-ChildItem -Path "$repoRoot/app/view" -Recurse -Filter '*.php' |
    Select-String -Pattern '<script\b(?![^>]*\bsrc=)' -AllMatches |
